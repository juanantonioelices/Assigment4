class EvaluationsController < ApplicationController
  ...
  def show
    ...
    @skeleton = Skeleton.find(@evaluation.skeleton)
    ...
    respond_to do |format|
      if @evaluation.save
        skeleton.topics.each do |topic|
          TopicValue.new(:evaluation_id =>@evaluation.id, :topic_id=>topic.id).save
          topic.subtopics.each do |subtopic|
            SubtopicValue.new(:evaluation_id =>@evaluation.id, :subtopic_id=>subtopic.id).save
            subtopic.subsubtopics.each do |subsubtopic|
              SubsubtopicValue.new(:evaluation_id =>@evaluation.id, :subsubtopic_id=>subsubtopic.id).save
            end
          end          
        end
